...
class SpamGDA:
    ...
    def __init__(self, sigma_diagonal_multiplier=0.01) -> None:
        super().__init__()
    ...
    def get_p_x_given_non_spam(self, features):
    ...
    def get_p_x_given_spam(self, features):
        common = 1 / (np.sqrt(np.power(2 * np.pi, self.d) * np.linalg.det(self.sigma)))
        sigma_inverse = np.linalg.pinv(self.sigma)
    ...
    def predict(self, features):
        p_predict_non_spam = self.get_p_x_given_non_spam(features) * self.p_non_spam
        p_predict_spam = self.get_p_x_given_spam(features) * self.p_spam

        return np.column_stack((p_predict_non_spam, p_predict_spam))
...
def demo_gda_on_spam_data():
    ...
    for k_fold_data in k_folds:
        ...
        training_predicted_labels = np.argmax(gda.predict(k_fold_data['training']['features']), axis=1)
        testing_predicted_labels = np.argmax(gda.predict(k_fold_data['testing']['features']), axis=1)
